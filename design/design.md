...
- **电影海报展示**：
  ...
  - 支持多选功能（复选框），用于批量操作
  - 显示用户个性化数据（评分、收藏状态）
  - 如果电影海报未能找到对应的文件，则使用images\default_poster作为默认海报
...
│   ├── login.html                # 登录页面
│   ├── profile.html              # 个人中心页面
│   ├── admin-dashboard.html       # 管理员后台首页（仪表盘）
│   ├── admin-movies.html          # 电影管理页面
│   ├── admin-users.html           # 用户管理页面
│   ├── admin-actors.html          # 演员管理页面
│   ├── admin-categories.html      # 分类管理页面
│   ├── admin-tags.html            # 标签管理页面
│   ├── admin-import.html          # 批量导入页面
...
│   │   ├── login.js              # 登录页脚本
│   │   ├── profile.js            # 个人中心脚本
│   │   ├── admin-dashboard.js    # 管理员仪表盘脚本
│   │   ├── admin-movies.js       # 电影管理脚本
│   │   ├── admin-users.js        # 用户管理脚本
│   │   ├── admin-actors.js       # 演员管理脚本
│   │   ├── admin-categories.js   # 分类管理脚本
│   │   ├── admin-tags.js         # 标签管理脚本
│   │   ├── admin-import.js       # 批量导入脚本
│   │   ├── admin-settings.js     # 系统设置脚本
│   │   └── utils.js              # 公共工具函数（token管理、请求封装等）
│   └── images/                   # 公共图片资源
│       └── avatars/              # 默认头像资源
├── routes/
│   ├── index.js                  # 路由入口
│   ├── auth.js                   # 认证API路由（登录、登出）
│   ├── movies.js                 # 电影API路由
│   ├── videos.js                 # 视频文件API路由
...
│   ├── admin-users.js            # 管理员-用户管理API路由
│   ├── admin-movies.js           # 管理员-电影管理API路由
│   ├── admin-actors.js           # 管理员-演员管理API路由
│   ├── admin-categories.js       # 管理员-分类管理API路由
│   ├── admin-tags.js             # 管理员-标签管理API路由
│   ├── admin-import.js           # 管理员-批量导入API路由
│   ├── admin-settings.js        # 管理员-系统设置API路由
│   ├── stats.js                  # 统计数据API路由
│   └── settings.js               # 设置API路由
├── services/
...
- order: 排序方向（asc, desc）
- category_id: 分类ID筛选
- search: 搜索关键词（电影名称、电影介绍、演员名称、电影ID、电影标签、发行日期）
...
- limit: 每页数量（默认50）
- search: 搜索关键词（演员名称）
- sort: 排序字段（name, movie_count, birthday）
- order: 排序方向（asc, desc）
    ...
    "actors": [
      {
        ...
        "birthday": "1980-01-15",
        "memo": "知名演员，代表作...",
        "photo_path": "/uploads/actors/actor-1.jpg",
        "movie_count": 10,
        "create_time": "2024-01-01"
      }
    ],
...
响应：
{
  ...
  "data": {
    ...
    "name": "演员名称",
    "birthday": "1980-01-15",
    "memo": "知名演员，代表作包括《电影A》、《电影B》等...",
    "photo_path": "/uploads/actors/actor-1.jpg",
    "movies": [
      {
        "movie_id": "movie-001",
        "title": "电影名称",
        "poster_path": "/movies/poster.jpg",
        "role": "主角",
        "release_date": "2024-01-01"
      }
    ],
...
响应：
{
  ...
  "data": {
    ...
    "birthday": "1980-01-15",
    "memo": "演员备注",
    "photo_path": "/uploads/actors/actor-15.jpg"
  },
  "message": "演员添加成功"
}
```
...
Body：
{
  "source_id": 2,          // 要合并的演员ID（将被删除）
  "target_id": 1           // 目标演员ID（保留）
}
...
响应：
{
  "success": true,
  "data": {
    "merged_count": 5,     // 合并的电影数量
    "target_actor": {
      "id": 1,
      "name": "演员名称",
      "movie_count": 15    // 合并后的电影数量
    }
  },
  "message": "演员合并成功"
}
...
管理员相关的API按照模块进行分类，接口路径统一以 `/api/admin/` 开头：

- **用户管理**：`/api/admin/users/*`
- **电影管理**：参考 4.3 节电影相关API（需要管理员权限的操作）
- **演员管理**：参考 4.7 节演员相关API（需要管理员权限的操作）
- **分类管理**：参考 4.4 节分类相关API（需要管理员权限的操作）
- **标签管理**：参考 4.6 节标签相关API（需要管理员权限的操作）
- **批量导入**：参考 4.3.13-4.3.14 节（需要管理员权限）
- **缓存管理**：`/api/admin/cache/*`
- **系统设置**：参考 4.11 节设置相关API（需要管理员权限）

所有管理员API都需要在请求头中携带有效的JWT Token，并且用户角色必须为 `admin`。
...
+------------------------------------------------------------------+
|  [工具栏]                                                          |
|  [MovieMGT图标]  首页  分类  管理(仅管理员)    [搜索框 🔍]  [Sign in] |
+------------------------------------------------------------------+
|  [统计栏]                                                          |
...
**工具栏说明（未登录状态）**：
- **站点图标**：MovieMGT，点击返回首页
- **导航菜单**：
  - **首页**：电影海报墙首页
  - **分类**：跳转到分类浏览页面（可选）
  - **管理**：管理员后台入口（仅管理员可见）
- **搜索框**：输入关键词后点击搜索图标🔍或按回车键搜索
- **Sign in**：未登录时显示"Sign in"，点击跳转到登录页面
...
- 搜索结果高亮匹配文字
- 无结果时显示友好提示
- 搜索范围：电影名称、电影介绍、演员名称、电影ID、电影标签、发行日期
...
个人中心页面复用首页的工具栏布局，不单独设计顶部导航栏。

```
+------------------------------------------------------------------+
|  [工具栏]                                                          |
|  [MovieMGT]  首页  分类  管理(仅管理员)  [搜索框 🔍]  [用户头像] ▼  |
+------------------------------------------------------------------+
|                                                                   |
|  +-------------------+  +-------------------------------------+  |
|  |   [用户头像]       |  | 基本信息                             |  |
...
**功能模块**：
- **工具栏**：复用首页的工具栏布局，包含站点图标、导航菜单、搜索框、个人中心下拉菜单
...
**侧边栏菜单**：
- **仪表盘**：系统概览、统计数据
- **电影管理**：电影列表、添加/编辑/删除电影
- **用户管理**：用户列表、创建用户、权限管理
- **演员管理**：演员列表、添加/编辑/删除演员
- **分类管理**：分类列表、添加/编辑/删除分类
- **标签管理**：标签列表、添加/编辑/删除标签
- **批量导入**：扫描目录、导入电影
- **系统设置**：系统配置、缓存管理
...
+------------------------------------------------------------------+
|                                                                   |
|  搜索: [________________] [搜索]     [高级筛选 ▼]                  |
|                                                                   |
|  分类: [全部▼]  标签: [全部▼]  年份: [全部▼]  状态: [全部▼]        |
|                                                                   |
+------------------------------------------------------------------+
|  [☐全选]                                                          |
+------------------------------------------------------------------+
| ☐ | 海报   | 电影名称     | 分类  | 年份  | 视频数 | 操作        |
+------------------------------------------------------------------+
| ☐ | [图]   | 电影A        | 动作  | 2024  | 3      | [编辑][删除]|
| ☐ | [图]   | 电影B        | 喜剧  | 2023  | 1      | [编辑][删除]|
| ☐ | [图]   | 电影C        | 科幻  | 2024  | 2      | [编辑][删除]|
+------------------------------------------------------------------+
|  已选择 0 部电影   [批量删除] [批量修改分类]                        |
|                                                                   |
|  显示 1-20 / 共156部    [<] [1] [2] [3] ... [8] [>]               |
+------------------------------------------------------------------+
```
...
+------------------------------------------------------------------+
|                                                                   |
|  搜索: [用户名/邮箱_____________] [搜索]                           |
|  角色: [全部▼]  状态: [全部▼]                                      |
|                                                                   |
+------------------------------------------------------------------+
| ID | 用户名    | 昵称     | 邮箱           | 角色   | 状态 | 操作        |
+------------------------------------------------------------------+
| 1  | admin     | 管理员   | admin@qq.com   | 管理员 | 激活 | [编辑]      |
| 2  | user001   | 张三     | zhang@qq.com   | 用户   | 激活 | [编辑][禁用]|
| 3  | user002   | 李四     | li@qq.com      | 用户   | 禁用 | [编辑][启用]|
| 4  | user003   | 王五     | wang@qq.com    | 用户   | 激活 | [编辑][禁用]|
+------------------------------------------------------------------+
|                                                                   |
|  显示 1-20 / 共45个用户   [<] [1] [2] [3] [>]                      |
+------------------------------------------------------------------+
```
...
| ☐ | 照片   | 演员名称   | 生日       | 电影数 | 操作            |
+------------------------------------------------------------------+
| ☐ | [图]   | 张三       | 1980-05-15 | 15    | [编辑][删除]    |
| ☐ | [图]   | 李四       | 1975-08-20 | 23    | [编辑][删除]    |
| ☐ | [默认] | 王五       | -          | 8     | [编辑][删除]    |
+------------------------------------------------------------------+
|  已选择 0 个演员   [批量删除] [合并演员]                            |
|                                                                   |
|  显示 1-20 / 共120位演员  [<] [1] [2] [3] ... [6] [>]              |
+------------------------------------------------------------------+
```
...
|                                                                   |
+------------------------------------------------------------------+
| ID | 分类名称 | 描述         | 颜色     | 电影数 | 排序 | 操作    |
+------------------------------------------------------------------+
| 1  | 动作     | 动作电影     | 🟥 #FF5722 | 45    | 1    | [编辑][删除] |
| 2  | 喜剧     | 喜剧电影     | 🟨 #FFC107 | 32    | 2    | [编辑][删除] |
| 3  | 科幻     | 科幻电影     | 🟦 #03A9F4 | 28    | 3    | [编辑][删除] |
| 4  | 恐怖     | 恐怖电影     | 🟪 #9C27B0 | 18    | 4    | [编辑][删除] |
+------------------------------------------------------------------+
|                                                                   |
|  提示：删除分类后，该分类下的电影将变为"未分类"状态                 |
+------------------------------------------------------------------+
```
...
+------------------------------------------------------------------+
|                                                                   |
|  搜索: [标签名称_______________] [搜索]                            |
|                                                                   |
+------------------------------------------------------------------+
| ID | 标签名称 | 颜色     | 电影数 | 操作            |
+------------------------------------------------------------------+
| 1  | 经典     | 🟨 #FFD700 | 56    | [编辑][删除]    |
| 2  | 获奖     | ⬜ #C0C0C0 | 34    | [编辑][删除]    |
| 3  | 推荐     | 🟦 #00BCD4 | 45    | [编辑][删除]    |
| 4  | 待看     | 🟧 #FF9800 | 23    | [编辑][删除]    |
+------------------------------------------------------------------+
|                                                                   |
|  显示 1-20 / 共30个标签   [<] [1] [2] [>]                           |
+------------------------------------------------------------------+
...
+------------------------------------------------------------------+
|                                                                   |
|  扫描目录: [__________________________] [选择目录]                 |
|                                                                   |
|  [+开始扫描]                                                       |
|                                                                   |
+------------------------------------------------------------------+
...
|  基本信息                                                          |
|  +-------------------------------------------------------------+ |
|  | 电影库目录: [__________________________] [选择]             | |
|  | 盒子目录: [__________________________] [选择]               | |
|  +-------------------------------------------------------------+ |
|                                                                   |
|  缓存管理                                                          |
|  +-------------------------------------------------------------+ |
...
```
+------------------------------------------------------------------+
|  [MovieMGT]    首页    分类    管理(仅管理员)    [搜索框 🔍]    Sign in  |
+------------------------------------------------------------------+
```
...
**站点图标（MovieMGT）**：
- 建议使用电影胶片或影院相关的图标
- 图标尺寸：建议 32x32 或 40x40 像素
- 格式：SVG 或 PNG（支持透明背景）
- 存放路径：`public/images/logo.png` 或 `public/images/logo.svg`

**Favicon**：
- 16x16 像素图标
- 存放路径：`public/images/favicon.ico`
